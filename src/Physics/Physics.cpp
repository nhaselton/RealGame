...
bool TestTriangleEllipse( SweepInfo* sInfo, Vec3 a, Vec3 b, Vec3 c ) {
	...
	for ( int i = 0; i < 3; i++ ) {
		...
		if ( SolveQuadratic( a, b, c, &x1 ) ) {
			...
			if ( !sInfo->foundCollision || dist < sInfo->eSpaceNearestDist ) {
				...
				//sInfo->normal = pos - vertices[i];
				sInfo->eSpaceIntersection = vertices[i];//pos + velocity * info->dist;//vertices[i];
				sInfo->t = x1;
			}
		}
	}
	...
	for ( int i = 0; i < 3; i++ ) {
		...
		if ( f0 >= 0.0f && f0 <= 1.0f ) {
			if ( !sInfo->foundCollision || x1 < sInfo->eSpaceNearestDist ) {
				//info->dist = x1;
				...
				//info->normal = triPlane.n;
				sInfo->eSpaceIntersection = vertices[ i  ] + f0 * edges[i];
				sInfo->eSpaceNearestDist = x1 * glm::length( sInfo->velocity );
				sInfo->foundCollision = true;
				sInfo->t = x1;
			}
		}
