...
void CreateEntityManager() {
	...
	for( u32 i = 0; i < MAX_PROJECTILES; i++ ) {
		entityManager.projectiles[i].index = -1;
		entityManager.projSparseList[i] = -1;
	}
}
...
Projectile* NewProjectile( Vec3 pos, Vec3 velocity, Vec3 radius, bool linear ) {
	...
	return 0;
#endif
	//Find an unused sparse item
	Projectile* projectile = &entityManager.projectiles[entityManager.numProjectiles];
	int sparseIndex = -1;

	//Get item out of sparse list. Set sparse list item to current ptr to projectile
	for( int i = 0; i < MAX_PROJECTILES; i++ ) {
		if( entityManager.projSparseList[i] == -1 ) {
			sparseIndex = i;
			entityManager.projSparseList[i] = entityManager.numProjectiles;
			break;
		}
	}
	if( sparseIndex == -1 ) {
		LOG_ASSERT( LGS_GAME, "Projectile sparse and dense list are not in sync. has dense but no sparse\n" );
		entityManager.numProjectiles--;
		return 0;
	}
...
void EntityManagerCleanUp() {
	for( int i = 0; i < entityManager.numRemoveProjectiles; i++ ) {
		...
		Projectile* toRemove = &entityManager.projectiles[removeDenseIndex];
		Projectile* replaceWith = &entityManager.projectiles[--entityManager.numProjectiles];

		entityManager.projSparseList[toRemove->index] = -1;
		entityManager.projSparseList[replaceWith->index] = removeDenseIndex;
		*toRemove = *replaceWith;
		int a = 0;
	}
	//for ( int i = 0; i < entityManager.numRemoveProjectiles; i++ )
...
void RemoveProjectile( Projectile* projectile ) {
	entityManager.removeProjectiles[entityManager.numRemoveProjectiles++] = (u16) projectile->index;
